...
# import libraries
library(haven)
library(stargazer) # for generating better analytical tables
library(Hmisc) # for summary statistics
library(chron) # for dates and times
library(lfe) # for estimating linear models with multiple group fixed effects

Exhaustion <- read_dta("C:/Users/Sakshi/Downloads/Working from home-20210525/Exhaustion.dta")
View(Exhaustion)
...
stargazer(data = df_exh, type = "text", align = TRUE,
          title = "Summary Statistics - Exhaustion", out = "C:/Users/Sakshi/Downloads/project.txt")
...
# changing grosswage to original scale (x1000)
df_exh<-within(df_exh,{grosswage2<-grosswage*1000})


# describe data
stargazer(data = df_exh, type = "text", align = TRUE,
          title = "Summary Statistics - Exhaustion", out = "C:/Users/Sakshi/Downloads/project1.txt")
...
# We can see from the density plot the exhaustion gap between treatment group and control group has increased after the experiment, which means the working from home has decreased employees exhaustion level.
# This is analyized later down.
...
# Analysis: Intuitively, higher education degree and grosswage are correlated, therefore part of the effect of grosswage on exhaustion is driven by their education level.
...
stargazer(em1, em2, em3, em4, em5, em6, type="text", omit=c("day","personid","year_week"), align=TRUE, title="Regression Models", out = "C:/Users/Sakshi/Downloads/project2.txt", label ="table:models", omit.stat=c("f", "ser"))
...
# H1: Employees with higher grosswage are less exhausted (control: individual, group, experiment, time)
fem4<-felm(lnexhaustion ~ grosswage | year_week + personid, data=df) # this variable won't change during experiment

# Analysis: Higher education degree and grosswage are correlated, therefore part of the effect of grosswage on exhaustion is driven by their education level
cor1<-lm(high_educ ~ grosswage, data=df) # this variable won't change during experiment
stargazer(cor1, type="text", omit=c("day","personid","year_week"), align=TRUE)


# H2: Male employees are more exhausted than women.
fem5<-felm(lnexhaustion ~ men | year_week + personid, data=df) # this variable won't change during experiment
em7<-lm(lnexhaustion ~ commute + experiment_expgroup + year_week + personid, data=df) # this variable won't change during experiment
 
stargazer(fem1, fem2, fem3, fem4, fem5, type="text", omit=c("day","personid","year_week"), align=TRUE)
